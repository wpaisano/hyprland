...
option_capture_1="Capturar Tudo"
option_capture_2="Capturar Tela Ativa"
option_capture_3="Capturar Seleção"
...
copy='Copiar'
save='Salvar'
copy_save='Copiar & Salvar'
edit='Editar'

# Wofi CMD com largura aumentada
wofi_cmd() {
    wofi --dmenu --replace --config ~/.config/wofi/config-screenshot.rasi --i --no-show-icons --lines 2 --width 50 --prompt "Captura de tela"
}

# Passar variáveis para o dmenu do wofi
run_wofi() {
    echo -e "$option_1\n$option_2" | wofi_cmd
}
...
# CMD
timer_cmd() {
    wofi --dmenu --replace --config ~/.config/wofi/config-screenshot.rasi --i --no-show-icons --lines 5 --width 50 --prompt "Escolha o timer"
}
...
####
# Escolher Tipo de Captura
# CMD
type_screenshot_cmd() {
    wofi --dmenu --replace --config ~/.config/wofi/config-screenshot.rasi --i --no-show-icons --lines 3 --width 50 --prompt "Tipo de captura"
}
...
####
# Escolher como salvar ou copiar a captura
# CMD
copy_save_editor_cmd() {
    wofi --dmenu --replace --config ~/.config/wofi/config-screenshot.rasi --i --no-show-icons --lines 4 --width 50 --prompt "Como salvar"
}
...
timer() {
    if [[ $countdown -gt 10 ]]; then
        notify-send -t 1000 "Captura de tela em ${countdown} segundos"
